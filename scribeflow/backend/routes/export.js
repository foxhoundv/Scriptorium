...
function collectDocuments(node, documents, result = []) {
  ...
  if (node.children) {
    for (const child of node.children) {
      if (child.type !== 'trash') {
        collectDocuments(child, documents, result);
      }
    }
  }
  return result;
...
function htmlToDocxParagraphs(html) {
  ...
  for (const block of blocks) {
    ...
    if (level) {
      ...
      // Handle inline bold/italic roughly
      const runs = [];
...
router.get('/:projectId/txt', async (req, res) => {
  try {
    ...
    for (const doc of docs) {
      ...
      output += htmlToPlainText(doc.content);
      output += '\n\n\n';
    }
...
router.get('/:projectId/md', async (req, res) => {
  try {
    ...
    for (const doc of docs) {
      output += `## ${doc.title}\n\n`;
      output += htmlToMarkdown(doc.content);
      output += '\n\n---\n\n';
    }
...
router.get('/:projectId/docx', async (req, res) => {
  try {
    ...
    for (const doc of docs) {
      ...
      allParagraphs.push(...htmlToDocxParagraphs(doc.content));
...
// Export as HTML (for printing/PDF via browser)
router.get('/:projectId/html', async (req, res) => {
  try {
    ...
    let body = '';
    for (const doc of docs) {
      body += `<section class="chapter"><h1 class="chapter-title">${doc.title}</h1><div class="chapter-content">${doc.content || ''}</div></section>`;
    }
